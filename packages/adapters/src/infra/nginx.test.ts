...
function makeExecutor(
  ...
  const exec = async (command: string): Promise<string> => {
    ...
    // The reload script itself legitimately uses `command -v systemctl`; do not
    // mistake that health check for the path probe.
    if (/^openresty\s+-V\b/.test(command)) throw new Error("no openresty in test");
