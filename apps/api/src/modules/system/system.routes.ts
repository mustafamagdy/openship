...
/* ── Kubernetes clusters ───────────────────────────────────────────
 * Clusters reuse registered servers as trusted kubectl execution points.
 * Discovery is read-only and org-scoped; no kubeconfig leaves the server. */
r.get(
  "/kubernetes/clusters",
  { tag: "server:list", collection: true, readOnly: true },
  kubernetesClusters.listKubernetesClusters,
);
