...
### Confirm the restarted service is healthy

The updater restarts the persistent service and waits for its health endpoint. A successful result means
the CLI, API, and matching dashboard are running the selected release. An attached
`openship up --foreground` process cannot restart itself; relaunch that process manually.
...
until `openship update` installs, restarts, and health-checks the selected release. See
