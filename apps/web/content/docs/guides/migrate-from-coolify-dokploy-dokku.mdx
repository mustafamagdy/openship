...
- Server-to-server always uses the **deploy path** (a downtime window). A locally-built image must first be
  published to an OCI registry; once the inspected workload uses that registry reference, the target pulls
  the exact digest with the connected registry credential. Builds that still use only a host-local
  `openship/*` image remain blocked.

To publish future builds automatically, open **Settings → General → Build artifact registry** and connect
GHCR or another OCI-compatible registry. OpenShip validates the credentials, encrypts the token at rest,
pushes each successful Docker build under an immutable deployment tag, stores the returned digest, and
deploys that digest. Disconnecting the registry does not delete previously published images.
