...
Updates the globally-installed CLI — which bundles the self-hosted server — from the selected GitHub
release source. A bare persistent service is restarted and its API health endpoint is probed. A Compose
installation is pulled and recreated without a follow-up health probe. Official upstream releases install
from the package registry; fork releases install their checksum-verified GitHub release bundle.
...
| `--set-source <channel>` | Persist `upstream`, `fork`, or `pinned`. |
| `--repo <owner/repo>` | GitHub repository used by `fork` and `pinned`. |
| `--version <version>` | Exact semantic version required by `pinned`. |
| `--show-source` | Print the effective source, including environment overrides. |
