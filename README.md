...
Point Openship at a source — a **GitHub repo**, an **Azure Repos repository**, a **local folder**, or a **prebuilt artifact** — and it runs one pipeline end to end:
...
5. **Push-to-deploy.** A GitHub webhook or Azure DevOps service hook re-runs the pipeline on every push to the tracked branch — rebuilding only the services a monorepo push actually touched.

### Azure Repos

Self-hosted Openship can connect private Azure Repos from **Settings → General → Azure Repos**. Add the Azure DevOps organization name and a personal access token with:

- **Code: Read** for project/repository discovery, branch and commit lookup, source inspection, and private HTTPS clones.
- **Service Hooks: Read & manage** to let Openship create and remove repository push subscriptions for auto-deploy.

The PAT is encrypted at rest and is resolved only for Azure DevOps API calls and clone operations. Azure push subscriptions use HTTPS plus a separate, per-repository Basic-auth secret generated by Openship; environments that track the same repository share the subscription but keep their own branch matching.
